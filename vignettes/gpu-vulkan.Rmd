...
## Restoring the default device

`ag_device()` and `ag_dtype()` set package-level state that outlives this
document: anything running later in the same R session keeps the GPU and the
f16 default unless it is put back. Restore it once the GPU section is done.
