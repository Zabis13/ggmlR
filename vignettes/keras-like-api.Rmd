...
```{r}
# ag_device()/ag_dtype() are package-level state, so a section that relies on a
# particular device has to say so rather than inherit whatever ran before it.
ag_device("cpu")
ag_dtype("f32")
...
ag_predict_cm <- function(model, x_cm, chunk = 64L) {
  n   <- ncol(x_cm)
  out <- matrix(0.0, nrow(as.matrix(model$forward(ag_tensor(x_cm[,1,drop=FALSE])))), n)
  for (s in seq(1L, n, by = chunk)) {
    e  <- min(s + chunk - 1L, n)
    lg <- as.matrix(model$forward(ag_tensor(x_cm[, s:e, drop = FALSE])))
