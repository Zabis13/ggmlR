...
* **Mamba backward on the GPU** — `SSM_CONV_BACK` and `SSM_SCAN_BACK` have Vulkan shaders (absent upstream), so training a state-space block no longer runs its forward on the GPU and its whole backward on the CPU. Training a Mamba block in `inst/examples/mamba_train_demo.R` drops from 21.7 s to 7.7 s, and the graph stops being split between backends. Requires `d_state` 128 or 256 and Mamba-2 shapes; anything else falls back to the CPU as before.
* **Float atomics are checked before use** — the Vulkan backend now queries `VK_EXT_shader_atomic_float` and its `shaderBufferFloat32AtomicAdd` feature, and refuses ops that need it on devices without it instead of dispatching a shader that cannot work. `GGML_VK_DISABLE_ATOMIC_FLOAT=1` forces the fallback. This also covers `scatter_elements` in add mode, which used the extension unguarded.
* **`inst/examples/backward_gpu_demo.R`** — reports, per backward op, which backend actually ran it, how long each took, and whether the two agree. A backward op that silently falls back to the CPU is otherwise invisible: the results stay correct and only the speed changes.
