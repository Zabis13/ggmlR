...
* **Fix (Windows/MinGW)**: silent crash during GPU buffer allocation that aborted Vulkan and meta-backend model loading (e.g. FLUX.1/FLUX.2 diffusion) with no error message. The internal `ggml_backend_buffer_init()` passed its `ggml_backend_buffer_i` interface (an ~88-byte struct of 11 function pointers) **by value** across the `extern "C"` translation-unit boundary; on MinGW this aborted the process at the call site. The interface is now passed by pointer (`const ggml_backend_buffer_i *`). The same fix covers the meta backend and the AMX CPU backend, which used the same by-value pattern. Reverse-dependency safe — `llamaR`/`sd2R` do not call this internal function directly (they must be rebuilt against the new static library to pick up the fix).
